...
function loadStatsPage() {
	...
	var cell_value; // current cell's value
	var currTeamName;
	var homeScore;
	var oppScore;
	var cuWins = 0;
	var cuLosses = 0;
	for(row_counter = 2; row_counter < table.rows.length; row_counter++) { // outer loop iterates over each row
		for(col_counter = 1; col_counter < table.rows[row_counter].cells.length; col_counter++) { // inner loop iterates over each column
			cell_value = table.rows[row_counter].cells[col_counter].innerHTML; //Read in a cells current value
			if(col_counter == 1) {
				currTeamName = cell_value;
			}
			if(col_counter == 2) {
				homeScore = parseInt(cell_value);
			}
			if(col_counter == 3) {
				oppScore = parseInt(cell_value);
			}
			if(col_counter == 4) {
				if(homeScore > oppScore) {
					table.rows[row_counter].cells[4].innerHTML = 'CU Boulder';
					table.rows[row_counter].cells[4].style.backgroundColor = 'gold';
					cuWins++;
				}
				else if (homeScore < oppScore) {
					table.rows[row_counter].cells[4].innerHTML = currTeamName;
					table.rows[row_counter].cells[4].style.backgroundColor = 'gainsboro';
					cuLosses++;
				}
			}
		}
