...
function loadWeatherData( ) {
  var query = getQuery();
  var url = `https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query=${query}`; // Place your weatherstack API Call Here - access_key to be used: 5bc82451636190abd9d7afe6fe9b20b5
  ...
  $.ajax({ url: url, dataType: "jsonp" }).then(function (data) {
    ...
    */
    ...
    // 2. location: This should be appended to the heading.
    document.getElementById('heading').innerHTML = "Today's Weather Forecast – " + data.location.name;
    // 3. temp_today
    var fTemp = getFarenheitTemp(data.current.temperature);
    if (fTemp < 85 && fTemp > 65) {
    ...
    document.getElementById('temp_today').innerHTML = `${fTemp}° F`;
    // 4. thermometer_inner
   ...
   document.getElementById('5_day_forecast').innerHTML = '';
    for(var i = 0; i < 5; i++) {
      ...
      `;
      document.getElementById('5_day_forecast').appendChild(card);
    }
  })
}
